...
import type ObsidianAlfredPlugin from '../main';
...
export class EmbeddedView extends nosuper(HoverPopover) {
	...
	static popoversForWindow(win?: Window) {
		return (
			Array.prototype.slice.call(
				win?.document?.body.querySelectorAll('.fs-leaf-view') ?? []
			) as HTMLElement[]
		)
	...
	static forLeaf(leaf: WorkspaceLeaf | undefined) {
		...
		const el =
			leaf && document.body.matchParent.call(leaf.containerEl, '.fs-leaf-view'); // work around matchParent race condition
	...
	constructor(
		...
		public plugin: ObsidianAlfredPlugin,
		waitTime?: number,
	...
	attachLeaf(): WorkspaceLeaf {
		...
		this.rootSplit.getContainer = () =>
			EmbeddedView.containerForDocument(this.document);
